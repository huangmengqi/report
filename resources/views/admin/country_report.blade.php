            ...
            <div class="page-header">
                ...
                <nav aria-label="breadcrumb">
                    <p class="card-description">
                        <a href="/admin/report/download?appid={{$appid}}&date={{$time1}}&gamename={{$gamename}}" class="btn btn-sm btn-gradient-success btn-icon-text">Export Excel</a>
                    </p>
                    <p class="card-description">
                        <a href="/admin/report/downloadmuti?appid={{$appid}}&date={{$time1}}&today={{$time2}}&gamename={{$gamename}}" class="btn btn-sm btn-gradient-success btn-icon-text">Export Muti Days Excel</a>
                    </p>
                </nav>
            </div>
            <div class="row">
...
<input type="text"  readonly style="width: 10rem;height: calc(2.875rem );margin-right: 20px"  class="form-control" name="date" id="demo" value="{{$time1}}  {{$time2}}" data-time1="{{$time1}}" data-time2="{{$time2}}"/>
                            ...
                            <!-- <table class="table table-bordered" > -->
                                ...
                                <div class="scroll-box">
                                    ...
                                    @foreach($list as $k=>$v)
                                        <tr>
                                            @if($countryid) <td>@php echo date('Y-m-d',$v->date);@endphp</td> @else <td>@php echo date('m-d',strtotime($time1)).'~'.date('m-d',strtotime($time2));@endphp</td> @endif
                                            <td>{{$v->english}}</td>
                                            ...
                                            <!-- @php 
                                                $days = intval((strtotime($time2) - strtotime($time1)) / 3600 / 24);  
                                                if($days == '0'){
                                                    $amecpm = $v->am_cpm;
                                                }else{
                                                    $amecpm = round(($v->am_cpm) / ($days+1),2);
                                                }
                                            @endphp -->
                                            ...
                                            <td>{{isset($v->upltv) ?$v->upltv : '0.00'}}</td>
                                            <td>{{isset($v->adx_revenue) ?$v->adx_revenue : '0.00'}}</td>
                                            <td>
                                                @php 
