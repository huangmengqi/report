...
@section('base')
        ...
        <div class="content-wrapper">
            <div class="row">
                <div class="col-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">请修改token信息</h4>
                            <form class="forms-sample" id="form">
                                <input class="form-control required" name="id" value={{$id}} type="hidden">
                                <div class="form-group">
                                    <label for="nickname">*token名称</label>
                                    <input type="text" required class="form-control required" name="name" value={{$token_name}} readonly="true">
                                </div>

                                <div class="form-group">
                                    <label for="nickname">*Token</label>
                                    <input type="text" required class="form-control required" name="token" placeholder="如果不需要修改，请点击下方取消返回">
                                </div>

                                <button type="button" onclick="commit({{$id}})" class="btn btn-sm btn-gradient-primary btn-icon-text">
    ...
    <script>
         function commit(id){
            ...
            myRequest("/admin/report/envupdate/"+id,"post",data,function(res){
                if(res.code == '200'){
                    layer.msg(res.msg)
                    setTimeout(function(){
