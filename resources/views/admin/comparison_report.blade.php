                                ...
                                <form class="forms-sample">
                                    <div class="form-group" style="width: 1000px">
                                        <div class="input-group col-xs-3">  
                                            <input type="text" style="width: 50px;height: calc(2.875rem + 2px);margin-right: 20px;flex: 0.5"  class="form-control required calendar1" @if($today) value="{{$today}}" @endif name="today" >
                                            <input type="text" style="width: 50px;height: calc(2.875rem + 2px);margin-right: 20px;flex: 0.5"  class="form-control required calendar1" @if($time2) value="{{$time2}}" @endif name="time2" >
                                            <select style="width: 50px;height: calc(2.875rem + 2px);margin-left: 20px" xm-select-height="38px" name="game" xm-select-placeholder="请选择应用" xm-select-search xm-select="selectId">
                                                @foreach($gamelisttotal as $k=>$v)
                                                    <option  @if(in_array($v->id,$gameid)) selected @endif value="{{$v->id}}">{{$v->name}}</option>
                                                @endforeach
                                            ...
                                            <select style="width: 50px;height: calc(2.875rem + 2px);margin-left: 20px" xm-select-height="38px" name="country" xm-select-placeholder="请选择国家" xm-select-search xm-select="selectId1">
                                                @foreach($countrylist as $key=>$value)
                                                    <option  @if(in_array($value->code_2,$countryid)) selected @endif value="{{$value->code_2}}">{{$value->code_2}}</option>
                                            ...
                                            <select class="form-control form-control-lg" name="top" style="width: 100px;height: calc(2.875rem + 2px);margin-right: 20px">
                                                ...
                                                <option  @if($top=='10') selected @endif value="10">默认前10条，不选请选空</option>
                                                <option  @if($top=='30') selected @endif value="30">top30</option>
                                                <option  @if($top=='50') selected @endif value="50">top50</option>
                                            </select>
                                ...
                                <table id="form-table" cellpadding="0" border="0" cellspacing="0">
                                    ...
                                    <tr class="head">
                                        ...
                                        @foreach($gamelist as $k1=>$v1)
                                            <th  data-type="num" colspan="3">{{$v1}}</th>
                                        @endforeach
                                    </tr>
                                ...
                                <div class="scroll-box">
                                    ...
                                    <tbody>                                       
                                        ...
                                        <tr>   
                                                ...
                                                <td><span style="color: green">{{isset($v2[$j]->am_cpm) ?$v2[$j]->am_cpm : '0.00'}}</span></td>
                                                <td>
