...
class Ctrunitcheck extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ctrunintcheck';
     ...
     * @var string
     */
    protected $description = 'Today Admob Unit Click Data check';
    ...
    public function handle()
    {
        ...
        foreach ($list as $key => $value) {
            ...
            if($ctr > 0.15){
                //重新拉取一遍数据，确保拉取数据正常
                $gamelist = DB::table('mg_game')->where('status',1)->where('cate',1)->where('id',$value->id)->get();
                // dump($gamelist);exit;
